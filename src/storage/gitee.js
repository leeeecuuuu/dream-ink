...
export const gitee = {
    get token() { return ls('nanscript_gitee_token') || ''; },
    get gistId() { return ls('nanscript_gitee_gist_id') || ''; },
    ...
    async findExistingGist() {
        const res = await fetch(`${GITEE_API_BASE}/gists?access_token=${this.token}&page=1&per_page=100`);
    ...
    async getGist() {
        if (!this.gistId) throw new Error('未关联 Gist ID');
        const res = await fetch(`${GITEE_API_BASE}/gists/${this.gistId}?access_token=${this.token}`);
    ...
    async uploadAll() {
        const files = {
            ...
            'config.json': { content: JSON.stringify({
    ...
    async downloadAll(callbacks = {}) {
            ...
            remote.forEach(rf => {
                if (localMap.has(rf.folderName)) {
                    ...
                    rf.prompts.forEach(p => {
                        if (!existingKeys.has(`${p.name}|${p.content}`)) existing.prompts.push(p);
                    });
        ...
        if (files['config.json']) {
            ...
            if (remote.baseUrl && $('baseUrl')) $('baseUrl').value = remote.baseUrl;
            if (remote.apiKey && $('apiKey')) $('apiKey').value = remote.apiKey;
            if (remote.modelGemini && $('modelGemini')) $('modelGemini').value = remote.modelGemini;
            if (remote.modelOpenai && $('modelOpenai')) $('modelOpenai').value = remote.modelOpenai;
