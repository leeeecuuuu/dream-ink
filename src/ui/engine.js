...
import { state, PROVIDER_DEFAULTS, GEMINI_MODELS } from '../state/app-state.js';
...
export function updatePreview() {
  ...
  if (preview) {
    if (state.currentEngine === 'openai') {
      ...
      const modelShort = model ? model.name : 'Banana';
      const ratioDisplay = state.geminiUseCustom
        ? `${state.geminiCustomWidth || '?'}×${state.geminiCustomHeight || '?'}`
        : (bananaRatio?.value || '1:1');
      const sizeDisplay = state.geminiUseCustom
        ? '自定义'
        : (bananaSize?.value || '1K');
      preview.textContent = `${modelShort} · ${ratioDisplay} | ${sizeDisplay} | x${b.value}`;
    }
  }
...
export function switchEngine(engineKey, silent = false) {
  ...
  if (engineKey === 'gemini') {
    ...
    if (mg && !mg.value.trim()) {
      const geminiModel = GEMINI_MODELS[state.geminiModelKey] || GEMINI_MODELS['nano-banana-2'];
      mg.value = geminiModel.modelName;
      ls('nanscript_modelGemini', geminiModel.modelName);
    }
  } else {
...
export function initEngine() {
  ...
  if (mg) {
    ...
    // 若有保存值则使用；否则根据 Gemini 子模型 key 查找对应模型名
    if (savedMg) {
      mg.value = savedMg;
    } else {
      const geminiModel = GEMINI_MODELS[state.geminiModelKey] || GEMINI_MODELS['nano-banana-2'];
